...
MongoClient.connect(url, { useUnifiedTopology: true }).then(client => {
    ...
    //why does this not need "return anymore, now that there's a .catch just before it? (and it's outside of the .then)"
    dboper.insertDocument(db, { name: "Breadcrumb Trail Campground", description: "Test" }, 'campsites')
        ...
        .then(result => {
            ...
            //why does this need a return statement? It ran ok without one and does so below in the .catch.
            return client.close();
        })
        .catch(err => {
            console.log(err);
